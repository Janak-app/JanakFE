    ...
    <div className="min-h-screen bg-[#F5F5F7]">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mx-4 mt-4 bg-white border border-[#E5E7EB] rounded-xl p-5 flex flex-col items-center">
          <div className="w-[72px] h-[72px] rounded-full bg-accent flex items-center justify-center mb-3">
            <span className="text-white font-bold text-2xl">{initials}</span>
          </div>
          <p className="text-lg font-bold text-[#111827]">{user?.email}</p>
          <p className="text-xs font-medium text-[#6B7280] mt-1">{roleLabel}</p>
          <div className="flex w-full mt-4 pt-4 border-t border-[#E5E7EB]">
            ...
            <div className="flex-1 flex flex-col items-center">
              ...
              <span className="text-xs font-semibold text-[#111827]">—</span>
            </div>
          </div>
        </div>
            ...
            const content = (
              <div className={`flex items-center gap-3 px-3.5 py-3.5 ${isLast ? "" : "border-b border-[#E5E7EB]"}`}>
                <div className={`w-8 h-8 rounded-[10px] flex items-center justify-center ${"danger" in m && m.danger ? "bg-[#FEE2E2]" : "bg-[#E0F2FE]"}`}>
                  <Icon className={`w-4 h-4 ${"danger" in m && m.danger ? "text-[#DC2626]" : "text-accent"}`} />
                </div>
                <div className="flex-1">
        ...
        {/* Legal */}
        <div className="mx-4 mt-3 bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
          <Link href="/privacy-policy" className="flex items-center gap-3 px-3.5 py-3.5 border-b border-[#E5E7EB]">
            <div className="w-8 h-8 rounded-[10px] bg-[#E0F2FE] flex items-center justify-center">
              <ShieldCheck className="w-4 h-4 text-accent" />
            </div>
            <p className="flex-1 text-sm font-semibold text-[#111827]">Privacy Policy</p>
            <ChevronRight className="w-4 h-4 text-[#9CA3AF]" />
          </Link>
          <Link href="/terms" className="flex items-center gap-3 px-3.5 py-3.5">
            <div className="w-8 h-8 rounded-[10px] bg-[#E0F2FE] flex items-center justify-center">
              <ScrollText className="w-4 h-4 text-accent" />
            </div>
            <p className="flex-1 text-sm font-semibold text-[#111827]">Terms &amp; Conditions</p>
            <ChevronRight className="w-4 h-4 text-[#9CA3AF]" />
          </Link>
        </div>
        ...
        <div className="mx-4 mt-3 bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
          ...
          >
            <div className="w-8 h-8 rounded-[10px] bg-[#FEE2E2] flex items-center justify-center">
              <LogOut className="w-4 h-4 text-[#DC2626]" />
            </div>
            <p className="flex-1 text-sm font-semibold text-[#DC2626]">Logout</p>
          </button>
        </div>
